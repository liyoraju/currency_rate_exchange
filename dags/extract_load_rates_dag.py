...
def etl_rates():
    ...
    @task.bash
    def transform():
        return "cd /opt/airflow/dbt_currency && dbt run --select silver_rates && dbt test --select silver_rates && dbt run --select gold_daily_rates && dbt test --select gold_daily_rates && dbt run --select gold_usd_inr && dbt test --select gold_usd_inr"
